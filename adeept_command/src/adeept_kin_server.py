...
def handle_inverse_kinematics(req):
    ...
    if phi == 0 and theta == 0 and psi == 0:
        ...
        try:
            ...
            wrist_y = y
            wrist_z = z
            link4_ang = atan2(pen, link4d)
            link4d = sqrt(link4d**2 + pen**2)
            ...
            q3_1 = round(- pi/2 - temp3_1 - link4_ang, 4)
            q3_2 = round(- pi/2 - temp3_2 - link4_ang, 4)
    ...
    # Ask for service and give a specific orientation
    else:
        cphi = cos(phi)
        sphi = sin(phi)
        ctheta = cos(theta)
        ...
        cpsi = cos(psi)
        spsi = sin(psi)
        R_04 = np.array([[cphi*ctheta, -sphi*cpsi+cphi*stheta*spsi, sphi*spsi+cphi*stheta*cpsi],
        ...
        try:
            ...
            R_03_1 = np.matmul(np.matmul(A1_1, A2_1), A3_1)[0:3, 0:3]

            A1_2 = ma2np(homo_matrix_2.A1)
            A2_2 = ma2np(homo_matrix_2.A2)
            A3_2 = ma2np(homo_matrix_2.A3)
            R_03_2 = np.matmul(np.matmul(A1_2, A2_2), A3_2)[0:3, 0:3]

            A4 = ma2np(homo_matrix_2.A4)[0:3, 0:3]

            # Determine which solution matches the input pose
            R_34_1 = np.matmul(np.transpose(R_03_1), R_04)
            R_34_2 = np.matmul(np.transpose(R_03_2), R_04)
...
def handle_forward_kinematics(req):
    ...
    transform = np.matmul(np.matmul(np.matmul(np.matmul(A1, A2), A3), A4), A5)
    x = transform[0, 3]
    y = transform[1, 3]
    z = transform[2, 3]
